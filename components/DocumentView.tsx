...
interface DocumentViewProps {
  ...
  highlightOnlyRisky?: boolean; // New prop to control highlighting
}
...
const DocumentView: React.FC<DocumentViewProps> = ({
  ...
  const findClauseInText = (
    ...
    if (start !== -1) {
      const actualStart = findActualPosition(
        documentText,
        normalizedDocument,
        start
      );
    ...
    const clauseWords = normalizedClause
      .split(" ")
    ...
    if (clauseWords.length > 0) {
      ...
      const lastWordIndex = normalizedDocument.indexOf(
        lastWord,
        firstWordIndex
      );
      ...
      if (
        ...
        const actualEnd =
          findActualPosition(documentText, normalizedDocument, lastWordIndex) +
          lastWord.length;
  ...
  const parts: DocumentPart[] = useMemo(() => {
    ...
    // Filter clauses based on highlightOnlyRisky setting
    const clausesToHighlight = highlightOnlyRisky
      ? clauses.filter(
          (clause) =>
            clause.riskLevel !== "Negligible" && clause.riskLevel !== "No Risk"
        )
      : clauses;
    ...
    clausesToHighlight.forEach((clause, index) => {
      ...
      if (match) {
        ...
        console.warn(
          `DocumentView: Could not match clause ${index}: "${clause.clauseText.substring(
            0,
            50
          )}..."`
        );
    ...
    console.log(
      `DocumentView: Matched ${matchedClauses.length}/${clausesToHighlight.length} clauses, result parts: ${result.length}`
    );

    return result;
  }, [documentText, clauses, highlightOnlyRisky]);
                ...
                const formattedText = part
                  ...
                  .map((line, lineIndex) => {
                    ...
                    if (isHeading) {
                      ...
                      let headingClass =
                        "text-xl font-bold text-gray-800 mt-8 mb-4 first:mt-0 bg-gray-100 px-4 py-2 rounded-lg border-l-4 border-indigo-500";
                      ...
                      if (/^\d+\.\s/.test(line)) {
                        ...
                        if (match) {
                          ...
                          if (level === 1) {
                            headingClass =
                              "text-2xl font-bold text-gray-900 mt-8 mb-4 first:mt-0 bg-indigo-50 px-4 py-3 rounded-lg border-l-4 border-indigo-600 shadow-sm";
                          } else if (level === 2) {
                            headingClass =
                              "text-xl font-bold text-gray-800 mt-6 mb-3 bg-blue-50 px-3 py-2 rounded-lg border-l-4 border-blue-500";
                          } else {
                            headingClass =
                              "text-lg font-semibold text-gray-700 mt-4 mb-2 bg-gray-50 px-3 py-1 rounded border-l-3 border-gray-400";
                          }
                        }
                      } else if (
                        line === line.toUpperCase() &&
                        ...
                        headingClass =
                          "text-3xl font-bold text-gray-900 mt-10 mb-6 first:mt-0 bg-gradient-to-r from-indigo-50 to-blue-50 px-6 py-4 rounded-xl border-l-6 border-indigo-600 shadow-md";
                      } else if (
                        /^(SECTION|CHAPTER|PART|ARTICLE|CLAUSE)/i.test(line)
                      ) {
                        // Keyword-based headings
                        headingClass =
                          "text-xl font-bold text-gray-800 mt-6 mb-3 bg-yellow-50 px-4 py-2 rounded-lg border-l-4 border-yellow-500";
                      }
