...
type View = 'login' | 'signup' | 'dashboard' | 'new_analysis' | 'analysis_result';
...
export default function App(): React.ReactElement {
  ...
  const handleStartAnalysis = useCallback(async (file: File, role: string) => {
    if (!currentUser) {
    ...
    try {
        ...
        if (!text || text.trim().length < 50) {
            throw new Error("Could not extract enough text from the document. The file might be empty, corrupted, or an image-based PDF without readable text.");
        }
        ...
        const newAnalysis = authService.saveAnalysis(currentUser.id, result, file.name, text);
  ...
  const renderContent = () => {
    ...
    switch(view) {
      ...
      case 'signup':
        return <SignupView onSignup={handleLogin} onSwitchToLogin={() => setView('login')} />;
      case 'dashboard':
        return currentUser ? <DashboardView user={currentUser} analyses={userAnalyses} onSelectAnalysis={handleViewAnalysis} onNewAnalysis={() => setView('new_analysis')} /> : null;
      case 'new_analysis':
         if (error) return <ErrorMessage message={error} onRetry={() => { setError(null); setView('dashboard');}} />;
         return <FileUpload onStartAnalysis={handleStartAnalysis} />;
      case 'analysis_result':
        if (error) return <ErrorMessage message={error} onRetry={() => { setError(null); setView('dashboard'); }} />;
        if (currentAnalysis) return <AnalysisDisplay analysisResult={currentAnalysis.analysisResult} documentText={currentAnalysis.documentText} onError={setError} onBackToDashboard={() => setView('dashboard')} />;
        return null;
    }
  };
  ...
  return (
    <div className="min-h-screen font-sans">
      <Header user={currentUser} onReset={handleReset} onLogout={handleLogout}/>
