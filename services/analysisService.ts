...
const analyzeChunk = async (chunk: string, role: string): Promise<ClauseAnalysis[]> => {
  ...
  const output = await chatComplete([
    { role: 'system', content: 'You are an expert legal AI assistant. Extract and analyze legal clauses from a text chunk. Return JSON only with a clauses array of objects: { clauseText, simplifiedExplanation, riskLevel in [Low, Medium, High, Negligible], riskReason, negotiationSuggestion }.' },
    { role: 'user', content: `${roleInstruction}\n\nTEXT:\n${chunk}\n\nReturn only valid JSON.` }
  ]);
...
const synthesizeReport = async (clauses: ClauseAnalysis[], role: string): Promise<{ summary: string; keyTerms: any[]; keyDates: any[]; missingClauses: MissingClause[] }> => {
  ...
  const output = await chatComplete([
    { role: 'system', content: 'You are an expert legal AI assistant. Synthesize a high-level report from pre-analyzed legal clauses. Return JSON only with: summary (string), keyTerms (array of {term, definition}), keyDates (array of {date, description}), missingClauses (array of {clauseName, reason}).' },
    { role: 'user', content: `${roleInstruction}\n\nANALYZED CLAUSES:\n${clauseContext}\n\nReturn only valid JSON.` }
  ]);
  const parsed = tryParseJson<{ summary: string; keyTerms: any[]; keyDates: any[]; missingClauses: MissingClause[] }>(output);
...
export const analyzeContract = async (
  ...
  const chunkResults = await Promise.all(chunks.map((chunk) => analyzeChunk(chunk, role)));
...
const cosineSimilarity = (a: number[], b: number[]): number => {
  ...
  for (let i = 0; i < a.length; i++) { const va = a[i]; const vb = b[i]; dot += va * vb; na += va * va; nb += vb * vb; }
  const denom = Math.sqrt(na) * Math.sqrt(nb) || 1; return dot / denom;
};

const vectorRetrieveRelevantChunks = async (chunks: string[], query: string, topK = 6): Promise<string[]> => {
  try {
    ...
    const scored = chunkVecs.map((vec, idx) => ({ idx, score: cosineSimilarity(vec, queryVec) })).sort((a, b) => b.score - a.score);
    const selected = scored.slice(0, Math.min(topK, scored.length)).map(s => chunks[s.idx]);
...
export const simulateImpact = async (documentText: string, scenario: string): Promise<string> => {
  ...
  const relevantChunks = await vectorRetrieveRelevantChunks(chunks, scenario, 6);
  ...
  const output = await chatComplete([
    { role: 'system', content: 'You are an expert legal AI assistant. Given contract excerpts and a hypothetical scenario, explain likely legal and financial consequences in clear, step-by-step language for a non-lawyer.' },
